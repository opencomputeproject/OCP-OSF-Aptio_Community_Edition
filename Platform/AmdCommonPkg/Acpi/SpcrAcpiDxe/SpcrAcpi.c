...
///
extern EFI_GUID  TerminalTypeGuid[];
  ...
  )
{
  ...
  if ((gSpcrInfo.BaseAddress.Address == 0x2F8) ||
      (gSpcrInfo.BaseAddress.Address == 0x2E8)) {
    // for COM1, COM3 --> IRQ 4 (Default value)
    // for COM2, COM4 --> IRQ 3
    gSpcrInfo.Irq = 0x3;
    gSpcrInfo.GlobalSystemInterrupt = 0x3;
  }
  // OEM info
  CopyMem (
  ...
  gSpcrInfo.Header.OemRevision     = 0;
