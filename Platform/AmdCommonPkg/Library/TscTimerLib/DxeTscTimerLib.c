...
/** @file
  DXE Timer Library implementation which uses the Time Stamp Counter in the processor.
  ...
  )
{
  ...
  //
  Status = EfiGetSystemConfigurationTable (&gAmdTscFrequencyGuid, (VOID **) &TscFrequency);
  ...
  *TscFrequency = InternalCalculateTscFrequency ();
  ...
  //
  gBS->InstallConfigurationTable (&gAmdTscFrequencyGuid, TscFrequency);
