  ...
  )
{
  ...
  MmioWrite32(
    (MmPciBase(DEFAULT_PCI_BUS_NUMBER_PCH, 0x22, 0x02) + 0x54),
    0x03
  );
  ...
  )
{
...
#endif
  PcdSet32S (PcdPcIoApicEnable, PcIoApicEnable);
  //
  ...
  PcdSet32S (PcdMemTsegSize, (0x400000 << SocketProcessorCoreConfig->TsegSize));
  if (SocketProcessorCoreConfig->IedSize > 0) {
    PcdSet32S (PcdMemIedSize, (0x400000 << (SocketProcessorCoreConfig->IedSize - 1)));
  } else {
    PcdSet32S (PcdMemIedSize, 0);
  }
...
{
  PcdSet64S (PcdUsb20OverCurrentMappings, (UINT64)(UINTN)Usb20OverCurrentMappings);
  PcdSet64S (PcdUsb30OverCurrentMappings, (UINT64)(UINTN)Usb30OverCurrentMappings);
}
...
{
  PcdSet64S (PcdIioBifurcationTable, (UINT64)(UINTN)mIioBifurcationTable);
  PcdSet8S (PcdIioBifurcationTableEntries, mIioBifurcationTableEntries);
  PcdSet64S (PcdIioSlotTable, (UINT64)(UINTN)mIioSlotTable);
  PcdSet8S (PcdIioSlotTableEntries, mIioSlotTableEntries);
}
...
{
  PcdSet64S (PcdAllLanesEparamTable, (UINT64)(UINTN)KtiTiogaPassAllLanesEparamTable);
  PcdSet32S (PcdAllLanesEparamTableSize, KtiTiogaPassAllLanesEparamTableSize);
}
